...
for i_part = 1:nparts
    ...
    i_event=2;
    while i_event < length(EEG.event) %skip the first
        if strcmp('48',EEG.event(i_event).type) == 1 && strcmp('41',EEG.event(i_event+2).type) == 0
            EEG.event(i_event+2)=[];
        end
        i_event = i_event+1;
    end
        ...
        nevents = length(exp.events(i_set,:));
            for i_event = 1:nevents
                ...
                for j_event = 1:nperevent
                     EEG.epoch(i_trial).eventcode(find(strcmp(num2str(exp.events{i_set,i_event}(j_event)),EEG.epoch(i_trial).eventtype))) = event_names(i_event);
                end
            end
        end
    end
    ...
    for i_trial = 1:EEG.trials
        for i_event = 1:length(exp.events(i_set,:));
            if isempty(find(strcmp(exp.event_names(i_event),EEG.epoch(i_trial).eventcode)== 1 & [EEG.epoch(i_trial).eventlatency{:}] > 0 ,1)) == 0
            output(i_part).target(i_trial) = str2num( EEG.epoch(i_trial).eventtype{ find(strcmp(exp.event_names(i_event),EEG.epoch(i_trial).eventcode)== 1 & [EEG.epoch(i_trial).eventlatency{:}] > 0 ,1) } );
            output(i_part).targetlatency(i_trial) = EEG.epoch(i_trial).eventlatency{ find(strcmp(exp.event_names(i_event),EEG.epoch(i_trial).eventcode)==1 & [EEG.epoch(i_trial).eventlatency{:}] > 0 ,1) };
            end
        end
    end
